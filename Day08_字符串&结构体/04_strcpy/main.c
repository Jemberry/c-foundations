...
void test3() {
	...
	// 所以，大家在用strcpy的时候，一定要注意，够不够装。 
	char *p1 = "123456789";

	char *p2 = "000";

	// p1指向只读数据段，所以，不要将数据写入到p1中。程序会直接报错。 
	strcpy(p1, p2);
}
...
void test4() {
	...
	// n 小于 strlen(src) + 1。 会拷贝n个字节。   不会额外补\0
	// n 等于  strlen(src) + 1。 会拷贝n个字节。  顺便拷贝了\0
	// n 大于  strlen(src) + 1。 会拷贝前面的字节，后面的字节直接填充 \0
}
